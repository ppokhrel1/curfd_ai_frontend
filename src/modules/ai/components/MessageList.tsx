...
const TypewriterEffect: React.FC<{ content: string }> = ({ content }) => {
  ...
  useEffect(() => {
    // If it's too short, just show it immediately to avoid jitter
    if (content.length < 5) {
...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, isLatest }) => {
  ...
  return (
    ...
    >
      {/* Avatar */}
      <div
        className={`flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border transition-all duration-300 ${
          isUser
      ...
      {/* Message Content */}
      {/* ✅ FIXED: min-w-0 prevents text from exploding the flex container */}
      <div
        className={`flex-1 min-w-0 max-w-[90%] sm:max-w-[85%] flex flex-col ${isUser ? "items-end" : "items-start"}`}
      >
        {!isUser && (
        ...
        <div
          className={`rounded-2xl px-4 py-3 transition-all duration-300 w-full ${
            isUser
        ...
        >
          {/* ✅ FIXED: break-words ensures long strings wrap */}
...
const FormattedContent: React.FC<{ content: string; isUser: boolean }> = ({
  ...
  return (
    <div
      className={`text-[13px] sm:text-[14px] leading-relaxed space-y-2 sm:space-y-3 ${
      ...
      {lines.map((line, index) => {
        ...
        if (line.startsWith("**") && line.endsWith("**")) {
          ...
          return (
            <h3
              key={index}
              className={`font-bold text-sm sm:text-[15px] pt-2 mb-1 ${
        ...
        if (line.trim().startsWith("```")) {
           // Skip code fence markers for simple rendering
          return null;
        }
        ...
        if (
          ...
          return (
            <div key={index} className="flex gap-2 sm:gap-3 items-start pl-1 group">
              <span
        ...
        if (/^\d+\./.test(line.trim())) {
          ...
          return (
            <div key={index} className="flex gap-2 sm:gap-3 items-start pl-1">
              <span
              ...
              <span className="flex-1 min-w-0 break-words opacity-90">
                {renderInlineFormatting(text, isUser)}
              </span>
            </div>
          );
        ...
        // Regular text
        return line.trim() ? (
          <p key={index} className="leading-relaxed opacity-95 break-words">
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith("`") && part.endsWith("`")) {
      return (
        <code
          key={i}
          className={`px-1.5 py-0.5 rounded text-[11px] sm:text-[12px] font-mono break-all ${
            isUser
