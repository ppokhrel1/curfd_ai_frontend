...
export const CADEditor: React.FC<CADEditorProps> = ({ className = "", onBuildComplete, onGenerateShape }) => {
  ...
  const SnippetBtn = ({ label, icon, snippet }: { label: string; icon: React.ReactNode; snippet: string }) => (
    <button
      ...
      title={`Insert ${label}`}
      className="flex items-center gap-1 px-1.5 py-1 rounded text-[10px] font-medium text-neutral-500 hover:text-neutral-800 hover:bg-neutral-200/60 transition-colors whitespace-nowrap flex-shrink-0"
    >
      {icon}
      <span>{label}</span>
    </button>
  ...
  return (
    <div className={`flex flex-col h-full bg-white ${className}`}>
      ...
      {/* ── Header ── */}
      <div className="flex-shrink-0 flex items-center justify-between px-3 py-2.5 border-b border-neutral-200 bg-neutral-50">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {!isRequirements && chatId && (
            <div className="flex items-center gap-1">
              {versions.length > 0 && (
                <div className="flex items-center gap-1 bg-white border border-neutral-200 rounded-lg px-1.5 py-1">
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-[10px] font-medium bg-neutral-100 hover:bg-neutral-200 text-neutral-600 hover:text-neutral-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          ...
          <button
            ...
            className="p-1.5 hover:bg-neutral-100 rounded-lg text-neutral-500 hover:text-neutral-800 transition-colors disabled:opacity-30"
          ...
          {!isRequirements && (
            <div className="flex items-center gap-1.5">
              ...
              <button
                onClick={() => exportStep()}
                disabled={isCompiling || !code.trim()}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wide bg-primary-700 hover:bg-primary-600 disabled:opacity-40 disabled:cursor-not-allowed text-white transition-all"
                title="Export as STEP file"
              >
                <Download className="w-3 h-3" />
                <span>STEP</span>
              </button>
              <div className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-neutral-50 border border-neutral-200 text-[10px] text-neutral-500">
                {compilePending
      ...
      {!isRequirements && (
        <div className="flex-shrink-0 border-b border-neutral-200/60 bg-neutral-50/25 px-1.5 py-0.5 overflow-x-auto scrollbar-none">
          <div className="flex items-center gap-0.5 w-max">
            {SHAPES.map((s) => <SnippetBtn key={s.label} {...s} />)}
            <div className="w-px h-3.5 bg-neutral-200/50 mx-1 flex-shrink-0" />
      ...
      {/* ── Editor Body ── */}
      <div className="flex-1 relative flex overflow-hidden font-mono group bg-[#1e1e2e]">
        {/* Line numbers — scroll synced with textarea */}
        <div
          ref={lineNumbersRef}
          className="flex-shrink-0 w-9 bg-[#1e1e2e] border-r border-[#2e2e3e] py-4 text-right pr-2 select-none text-[#6e6e8e] text-[11px] leading-6 overflow-hidden"
        >
        ...
        <textarea
          ...
          className="flex-1 bg-transparent py-4 px-3 outline-none resize-none text-neutral-100 text-[13px] leading-6 caret-blue-400 scrollbar-thin scrollbar-thumb-neutral-600 selection:bg-blue-500/30"
          placeholder={
        ...
        <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
          <div className="bg-[#2a2a3e]/90 backdrop-blur-sm border border-neutral-700 rounded-full px-2.5 py-1 flex items-center gap-1.5 text-[10px] text-neutral-400">
      ...
      {/* ── Footer ── */}
      <div className="flex-shrink-0 border-t border-neutral-200 bg-neutral-50/30 px-3 py-1.5 flex items-center justify-between">
        <div className="flex items-center gap-3 text-[10px]">
          <div className="flex items-center gap-1.5">
            {isCompiling || compilePending
              ? <Loader2 className="w-2.5 h-2.5 animate-spin text-blue-400" />
              : <div className={`w-1.5 h-1.5 rounded-full ${isDirty ? "bg-amber-500" : "bg-primary-500"}`} />
            }
          ...
          <div className="w-px h-3 bg-neutral-200" />
          <span className="text-neutral-500">{lineCount}L</span>
        </div>
